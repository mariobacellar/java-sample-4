...
/**
* GenerateDTDFromXML<BR>
* Generates a possible DTD from an XML document instance.
* Pure SAX version of the Saxon GenerateDTDFromXML
* The program has no remaining dependencies on Saxon; 
...
*/
public class GenerateDTDFromXML extends org.xml.sax.helpers.DefaultHandler {

	
	protected static int MIN_ENUMERATION_INSTANCES	= 10;		// Minimum number of appearances of an attribute for it to be considered a candidate for an enumeration type   
	...
	protected static int MIN_ENUMERATION_RATIO		= 3;		// An attribute will be regarded as an enumeration attribute only if the number of instances divided by the number of distinct values is >= this ratio   
	...
	protected static int MIN_ID_VALUES				= 10;		// Minumum number of attribute values that must appear for the attribute to be regarded as an ID value
	protected static int MAX_ID_VALUES				= 100000; 	// Maximum number of attribute values to be saved while checking for uniqueness

	// Alphabetical list of element types appearing in the document. Each has the element name as a key and an ElementDetails object as the value
	public TreeMap elementList;   
...
//		if (args.length != 1) {
//			System.err.println("Usage: java GenerateDTDFromXML input-file >output-file");
//			System.exit(1);
//		} 
		...
		GenerateDTDFromXML 
		app = new GenerateDTDFromXML();
		//app.run(args[0]);
		String filename = "C:\\java\\workspace\\xmlmanager\\src\\main\\resources\\data\\input\\dtd\\ListaAltContaFatura.dtd";
		app.run(filename);
		app.printDTD();
	}

	public GenerateDTDFromXML () {
	...
	* TODO: This is currently an incomplete test, it treats all non-ASCII characters as being valid in names.
	*/
	...
	* TODO: This is currently an incomplete test, it treats all non-ASCII characters as being valid in NMTOKENs.
	*/
	...
	private void printDTD () {
		...
		while ( e.hasNext() ) {
			...
			if (childKeys.size()>0 && !ed.hasCharacterContent) {
				...
				// All elements of this type have the same child elements in the same sequence, retained in the childseq vector
				if (ed.sequenced) {
				...
				// The children don't always appear in the same sequence; so list them alphabetically and allow them to be in any order
		...
		while ( a.hasNext() ) {
			...
			// if there is only one attribute value, and at least MIN_FIXED occurrences of it, treat it as FIXED 
			boolean isfixed = required && ad.values.size()==1 && ad.occurrences >= MIN_FIXED;

			// if the number of distinct values is small compared with the number of occurrences, treat it as an enumeration
	...
	* The escaped characters are written to the dest array starting at position 0; the number of positions used is returned as the result
	...
	* This static method converts a Unicode string to a string containing only ASCII characters, in which non-ASCII characters are represented by the usual XML/HTML escape conventions (for example, "&lt;" becomes "&amp;lt;").<br>
	* Note: if the input consists solely of ASCII or Latin-1 characters, the output will be equally valid in XML and HTML. Otherwise it will be valid only in XML.
	...
	public void startElement (String uri, String localName, String name, Attributes attributes) throws SAXException {
		...
		for (int a=0; a<attributes.getLength(); a++) {
			...
			if (!ad.values.contains(val)) {
				...
				// For economy, don't save the new value unless it's needed; it's needed only if we're looking for ID values or enumerated values
	...
	*/
	public void endElement (String uri, String localName, String name) throws SAXException {
		...
		// If the number of child element groups in this parent element is less than the number in previous elements, then the absent children are marked as optional
		ElementDetails ed = (ElementDetails) elementList.get(name);
	...
	*/
	...
	/**
	* ChildDetails records information about the presence of a child element within its parent element. 
	* If the parent element is sequenced, then the child elements always occur in sequence with the given frequency.
	*/
